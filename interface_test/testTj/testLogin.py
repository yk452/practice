...
class TestLogin:
    ...
    login_info = {'username': 'tj001', 'password': '666666'}
    ...
    def testLogin(self):
        ...
        login_info2={"username":"tj001",'password':jm}
        print(login_info2)
    ...
    def encrypt(self,pas,pwd):
        ...
        print(len(prand),sPos,sPos*5,"prand的长度")
        mult=int(prand[sPos]+prand[sPos*2]+prand[sPos*3]+prand[sPos*4]+prand[sPos*5-1])#获取prand相应位置的字符并转换为整数
        ...
        print(salt,"初次赋值的salt")
        prand+=str(salt)
        print(prand,"prand与取模的值的字符串连接后的prand")
        # 因js的parseInt方法与自动将长串数字使用科学计算的原因，所以在python中改成以下写法
        if len(prand)>10:
            kxjs=format(int(prand[10:len(prand)]),'.16e')
            kxjs_str=str(kxjs)[10:18]
            prand=int(kxjs_str)+1
        #print(prand,"取prand的前10与后面的值相加循环后的字符串prand，js逻辑，在python中已不适用")
        prand=(mult*prand+incr)%modu
        print(prand,"已为整数，不再是字符串")
        enc_str=""
        for k in range(0,len(pas)):
            ...
            if enc_chr < 16:
                enc_str += "0"+str(hex(enc_chr))[2:]#hex方法是将10进制转换为16进制
                print(enc_str,"字符串1",k)
            else:
                enc_str += str(hex(enc_chr))[2:]
