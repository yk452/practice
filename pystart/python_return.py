...
class StatusCodeType:
    ...
    def getStatusType(self, param):
        if(param.isdigit()==False):#str.isdigit()
            print(self.errorStr)
            return self.errorStr
        elif( int(param) <100 or int(param) >599):
            print(self.errorStr)
            return self.errorStr
        #上面return后下面两句就不执行了，跟js不一样
        print(self.normalStr.format(param,param[0]+"XX",self.msgDict[param[0]+"XX"]))
        return self.normalStr.format(param,param[0]+"XX",self.msgDict[param[0]+"XX"])

if __name__ == "__main__":
    StatusCodeType().getStatusType("323")